...
Once you have your .xml file, run this command from your redmine directory:
  ...
  rake redmine:migrate_from_google_code filename=path/to/myproject_google-code-export.xml project=myproject

To map Google Code users and statuses to Redmine users and statuses, edit config/mappings.yml and run:

  rake redmine:migrate_from_google_code filename=path/to/myproject_google-code-export.xml project=myproject mappings=plugins/redmine_xml_import/config/mappings.yml
